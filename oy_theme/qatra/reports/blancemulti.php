...
$where = "where mon_mt<>'$curr' AND mon_stat=0 group by mon_name";
$companies = $dbase->tbl2array('sob_money', 'mon_name', 'mon_name', $where);
	 ...
	 foreach ($companies as $comid => $comname) {
             ...
             $result = $dbase->query("SELECT mon_name,mon_eoe,mon_mt,sum(mon_rmoney) as moneyx FROM sob_money where mon_mt<>'$curr' AND mon_stat=0 and mon_name='$comid' group by mon_mt,mon_eoe");
             while($rec = $dbase->fetch_assoc($result))
            {

                 $mon1[$rec['mon_mt']][$rec['mon_eoe']]['rmon'] =   $rec['moneyx'];
             }
...
//                 
            ...
            foreach($oild as $id => $val){
                ...
                if($id==$curr){
                   ...
                   $mon[$id] = 0 ; //(($mon1[$id][2]['rmon'])+$imo+($mon1[$id][7]['rmon']))-($mon1[$id][1]['rmon']+$exo+($mon1[$id][5]['rmon'])); //($exo+$exm)-(($mon7[$id]+$mon2[$id])-($mon1[$id]+$mon5[$id]));
                }else{
...
<div class="container">	
    ...
    <table id="datatbl" class="table table-condensed" style="width:100%">
            <tr>
