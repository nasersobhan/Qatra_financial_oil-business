...
$tbl = TBL_PIX.'categories_oy';
	...
	// foreach ($companies as $comid => $comname) {
             ...
             $result = $dbase->query("SELECT mon_name,mon_eoe,mon_mt,(mon_rmoney) as moneyx FROM sob_money where mon_mt<>'$curr' AND mon_stat=0 group by mon_mt,mon_eoe");
             while($rec = $dbase->fetch_assoc($result))
            {
                 $comid=$rec['mon_name']; 
                 $mon1[$comid][$rec['mon_mt']][$rec['mon_eoe']]['rmon'] =   $rec['moneyx'];
                 
                    $oild =  cat_2arr_l('currency',0,'fa_AF');
                        unset($oild[$currid]);
                        foreach($oild as $id => $val){

                                   if(!isset($mon1[$comid][$id][2]['rmon'])){
                                        $mon1[$comid][$id][2]['rmon']=0;
                                    }
                                    if(!isset($mon1[$comid][$id][1]['rmon'])){
                                          $mon1[$comid][$id][1]['rmon']=0;
                                    }
                                    if(!isset($mon1[$comid][$id][5]['rmon'])){
                                          $mon1[$comid][$id][5]['rmon']=0;
                                    }
                                    if(!isset($mon1[$comid][$id][7]['rmon'])){
                                          $mon1[$comid][$id][7]['rmon']=0;
                                    }
                        }
            ...
            foreach($oild as $id => $val){
                $mon[$id]=0;$allcoms[$comid][$id]=0;
                $company=array();
                    $mon[$comid][$id] =(($mon1[$comid][$id][2]['rmon'])+($mon1[$comid][$id][7]['rmon'])) - (($mon1[$comid][$id][1]['rmon'])+($mon1[$comid][$id][5]['rmon'])); //(($mon7[$id]+$mon2[$id])-($mon1[$id]+$mon5[$id]));   
...
//                if($type=='mcompany'){
//			$result = $dbase->query("SELECT imp_eoe,imp_m_total FROM sob_impexp where imp_stat=0 and imp_m_name='$comid'");
//			$exo=0;$imo=0;
//			
...
//                            }         
//                   if($type=='ocompany'){            
//                                $result = $dbase->query("SELECT imp_eoe,imp_o_total FROM sob_impexp where imp_stat=0 and imp_o_name='$comid'");
//			$exo=0;$imo=0;
//			
//			while($row = $dbase->fetch_array($result))
//				{
...
//                                 }         
//                   if($type=='tcompany'){  
//                                   $result = $dbase->query("SELECT imp_eoe,imp_t_total FROM sob_impexp where imp_stat=0 and imp_t_cname='$comid'");
//			$exo=0;$imo=0;
//			
//			while($row = $dbase->fetch_array($result))
//				{
...
//                                }         
//                   if($type=='company'){   
//                                $result = $dbase->query("SELECT imp_eoe,imp_s_total FROM sob_impexp where imp_stat=0 and imp_name='$comid'");
//			$exo=0;$imo=0;
//			
//			while($row = $dbase->fetch_array($result))
//				{
...
<div class="container">	
        ...
        <table id="datatbl">
            <tr>
                <td class="title" colspan="4"><h1>بلاکس کلی شرکتها نظر به نوع پول</h1></td>
            </tr>
             <tr  class="tox" >
                <th width="25%">نام شرکت</th>
                ...
                foreach($oild as $id =>$val){
                    $total[$id]=0;
                     $total2[$id]=0;
                    echo '<th width="25%">' . get_cate_name($val) .  '</th>';
                }
                ?>
