...
if(is_post('oy_form_validate')){
       ...
       // $data = $_POST;
        ...
        $data[$fld_pre . 'parent'] =is_post($fld_pre .'parent');
         $data[$fld_pre . 'category'] =is_post($fld_pre .'category');
        if(is_get('val'))
...
global $add_form,$excp, $dbase;
   ...
   $edit = FALSE;
    ...
    $fields = 'S:'.  g_lbl('sub-title').'::S,name,desc,category,parent,lang,S:'.  g_lbl('thumb').'::S,avatar,tags';
    $add_form->set_flds($fields);
    $add_form->submit = '<tr><td></td><td><button type="submit" class="btn btn-info">'.g_lbl('save').'</button></td></tr>';
     $cates = cat_2arr_l('location_type',0,'fa_AF'); // $dbase->tbl2array(TBL_PIX . 'categories_oy', 'cat_id', 'cat_name', " WHERE cat_lang='{$lang}' AND cat_type = 'deals' AND cat_parent='{$parent}' AND ((cat_uid=0) OR (cat_uid=" . user_uid() . "))");
    $excp['category'] = array('label' => g_lbl('category'),'options'=>$cates,'att'=>'data-type="location_type"', 'type' => 'select', 'class' => 'form-control');
   ...
   // $excp['lang'] = array('label' => g_lbl('language').':','name'=>'content', 'type' => 'area', 'att' => ' rows="7" ', 'class' => 'form-control editor');
    ...
    $cats[0] = 'No Parent';
    $typearr = cat_2arr_l($type);
    $excp['parent'] =  array('label' => g_lbl('parent').':','type' => 'select',  'options' => $typearr, 'def' => 0,'class'=>'form-control ', 'att' => ' id="" data-type=""  title="Please select a parent ..." ');
